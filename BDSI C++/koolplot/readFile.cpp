...
struct inflectionRes{
    double C;

};
inflectionRes findInflectionPoint(vector<double> YA, vector<double> XA);

vector <double> smooth(vector<double> foo);
vector<int> findMaxima(vector<double>foo);
...
int main () {
    ...
    inflectionRes newStruct;
    newStruct = findInflectionPoint(Y_sorted,X_sorted);
    double C = newStruct.C;
...
inflectionRes findInflectionPoint(vector<double> YA, vector<double> XA){
    	...
    	lastLocalMaxIndex = localMax[localMax.size()-1];
